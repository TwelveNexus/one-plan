...
COPY backend/organization-service/build.gradle .
COPY backend/organization-service/settings.gradle .
COPY backend/commons/ /app/commons/

# Copy source code
COPY backend/organization-service/src ./src
...
# Expose the port (Organization Service runs on 8083)
EXPOSE 8083
