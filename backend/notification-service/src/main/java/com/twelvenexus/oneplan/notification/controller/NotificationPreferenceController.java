...
public class NotificationPreferenceController {
  ...
  public ResponseEntity<NotificationPreferenceDto> createOrUpdatePreference(
    ...
    NotificationPreference preference =
        preferenceService.createOrUpdatePreference(
            userId, tenantId, dto.getNotificationType(), dto.getEnabledChannels(), dto.isEnabled());
  ...
  public ResponseEntity<List<NotificationPreferenceDto>> getUserPreferences(
    ...
    List<NotificationPreference> preferences =
        preferenceService.getUserPreferences(userId, tenantId);

    return ResponseEntity.ok(preferences.stream().map(this::toDto).collect(Collectors.toList()));
  }
  ...
  private NotificationPreferenceDto toDto(NotificationPreference preference) {
    ...
    dto.setQuietTimeStart(
        preference.getQuietTimeStart() != null
            ? preference.getQuietTimeStart().toLocalTime().toString()
            : null);
    dto.setQuietTimeEnd(
        preference.getQuietTimeEnd() != null
            ? preference.getQuietTimeEnd().toLocalTime().toString()
            : null);
    return dto;
  }
}
