...
public interface NotificationRepository extends JpaRepository<Notification, UUID> {
  ...
  Page<Notification> findByUserIdAndTenantIdAndStatus(
      UUID userId, UUID tenantId, NotificationStatus status, Pageable pageable);

  @Query(
      "SELECT n FROM Notification n WHERE n.userId = :userId AND n.tenantId = :tenantId AND n.readAt IS NULL")
  Page<Notification> findUnreadByUserIdAndTenantId(UUID userId, UUID tenantId, Pageable pageable);

  @Query(
      "SELECT COUNT(n) FROM Notification n WHERE n.userId = :userId AND n.tenantId = :tenantId AND n.readAt IS NULL")
  long countUnreadByUserIdAndTenantId(UUID userId, UUID tenantId);

  List<Notification> findByStatusAndScheduledAtLessThanEqual(
      NotificationStatus status, LocalDateTime scheduledTime);

  List<Notification> findByStatusAndRetryCountLessThan(
      NotificationStatus status, Integer maxRetries);

  @Query(
      "SELECT n FROM Notification n WHERE n.userId = :userId AND n.tenantId = :tenantId "
          + "AND n.type IN :types AND n.createdAt BETWEEN :startDate AND :endDate")
  List<Notification> findForDigest(
      UUID userId,
      UUID tenantId,
