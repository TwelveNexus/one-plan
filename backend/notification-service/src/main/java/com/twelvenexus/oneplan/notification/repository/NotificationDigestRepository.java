...
public interface NotificationDigestRepository extends JpaRepository<NotificationDigest, UUID> {
    ...
    @Query("SELECT d FROM NotificationDigest d WHERE d.userId = :userId AND d.tenantId = :tenantId " +
           "AND d.sentAt BETWEEN :startDate AND :endDate")
    List<NotificationDigest> findByUserIdAndSentDateRange(UUID userId, UUID tenantId, 
