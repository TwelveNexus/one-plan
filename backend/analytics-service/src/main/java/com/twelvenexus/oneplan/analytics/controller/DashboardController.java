...
public class DashboardController {
  ...
  public ResponseEntity<DashboardDto> createDashboard(
    ...
    Dashboard dashboard =
        dashboardService.createDashboard(
            tenantId, dto.getName(), dto.getDescription(), userId, dto.isPublic());
  ...
  public ResponseEntity<DashboardDto> updateDashboard(
      @PathVariable UUID dashboardId, @Valid @RequestBody CreateDashboardDto dto) {
    Dashboard dashboard =
        dashboardService.updateDashboard(
            dashboardId, dto.getName(), dto.getDescription(), dto.isPublic());
  ...
  public ResponseEntity<DashboardDto> getDashboard(
      @PathVariable UUID dashboardId, @RequestHeader("X-Tenant-Id") UUID tenantId) {
  ...
  public ResponseEntity<List<DashboardDto>> getUserDashboards(
      @RequestHeader("X-Tenant-Id") UUID tenantId, @RequestHeader("X-User-Id") UUID userId) {
    List<Dashboard> dashboards = dashboardService.getUserDashboards(tenantId, userId);
    return ResponseEntity.ok(dashboards.stream().map(this::toDto).collect(Collectors.toList()));
  }
  ...
  public ResponseEntity<DashboardWidgetDto> addWidget(
      @PathVariable UUID dashboardId, @Valid @RequestBody CreateWidgetDto dto) {
  ...
  public ResponseEntity<DashboardWidgetDto> updateWidget(
      @PathVariable UUID widgetId, @Valid @RequestBody CreateWidgetDto dto) {
    DashboardWidget widget =
        dashboardService.updateWidget(widgetId, dto.getTitle(), dto.getConfiguration());
  ...
  private DashboardDto toDto(Dashboard dashboard) {
    ...
    if (dashboard.getWidgets() != null) {
      dto.setWidgets(
          dashboard.getWidgets().stream().map(this::toWidgetDto).collect(Collectors.toList()));
    }
