...
public class PaymentMethodController {
  ...
  public ResponseEntity<PaymentMethodDto> savePaymentMethod(
      @RequestHeader("X-Tenant-Id") UUID tenantId, @Valid @RequestBody CreatePaymentMethodDto dto) {
    PaymentMethod paymentMethod =
        paymentService.savePaymentMethod(
            tenantId, dto.getGateway(), dto.getType(), dto.getDetails());
  ...
  public ResponseEntity<List<PaymentMethodDto>> getTenantPaymentMethods(
    ...
    return ResponseEntity.ok(methods.stream().map(this::toDto).collect(Collectors.toList()));
  }

  @PutMapping("/{paymentMethodId}/default")
  @Operation(summary = "Set default payment method")
  public ResponseEntity<Void> setDefaultPaymentMethod(@PathVariable UUID paymentMethodId) {
