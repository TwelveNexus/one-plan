...
public interface PaymentRepository extends JpaRepository<Payment, UUID> {
    ...
    @Query("SELECT p FROM Payment p WHERE p.tenantId = :tenantId AND p.paidAt BETWEEN :startDate AND :endDate")
    List<Payment> findPaymentsByDateRange(@Param("tenantId") UUID tenantId,
                                         @Param("startDate") LocalDateTime startDate,
                                         @Param("endDate") LocalDateTime endDate);

    @Query("SELECT SUM(p.amount) FROM Payment p WHERE p.tenantId = :tenantId AND p.status = 'COMPLETED' AND p.paidAt BETWEEN :startDate AND :endDate")
    Double getTotalRevenue(@Param("tenantId") UUID tenantId,
                          @Param("startDate") LocalDateTime startDate,
                          @Param("endDate") LocalDateTime endDate);
}
